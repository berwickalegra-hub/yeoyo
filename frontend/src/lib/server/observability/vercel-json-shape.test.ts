...
// (5 Phase-5 canonical + 1 post-audit email-job-purge + 1 Task 11 chariow-reconcile.)
//
...
describe('vercel.json schema (CRON-07, D-20)', () => {
  ...
  it('declares schedules for the 7 canonical crons (Phase 5 + post-audit + Task 11)', () => {
    if (!existsSync(VERCEL_JSON)) return;
    const cfg = JSON.parse(readFileSync(VERCEL_JSON, 'utf8')) as VercelConfig;
